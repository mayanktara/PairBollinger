...
with open(string1, "r") as filestream:
    ...
    pnl.remove(pnl[0])
    meandaily=np.mean(pnl)
    deviation=np.std(pnl)
    print(deviation)
    print(len(pnl))
    n = len(finPnl)
    drawdown = 0
...
print(minpnl,",",maxpnl,",",drawdown,",",cummPnl,",",minTradePnl,",",maxTradePnl,",",positiveTrades,",",negativeTrades,",",meandaily,",",deviation)
