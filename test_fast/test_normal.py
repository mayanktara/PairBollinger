...
data_df=pd.read_csv('../data/finaldata2018.csv')
print(data_df)
holiday_df=pd.read_csv('../data/holiday2018.csv')
print(holiday_df)
...
with open('trade_completed.csv', 'a') as ftrades:
    ...
    for index, row in tradedPairs_df.iterrows():
        ...
        totalMon=(((price1*quantity1)+(price2*quantity2))*1)/100
        """
        if ((cpOd1 > val1) and (val1 > cpT1)):
            if ((cpOd2 < val2) and (val2 < cpT2)):
                ftrades.write(f"{symbol1},{symbol2},{cpT1},{cpT2},{price1},{price2}\n")
                print("trade")
            else :
                with open('traded_pairs.csv', 'a') as fpair:
                    fpair.write(f"{symbol1},{symbol2},{price1},{price2}\n")
