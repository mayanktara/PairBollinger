...
while res_date <= end:
    check=pd.to_datetime(res_date)
    print(res_date,check)
    res_date += datetime.timedelta(days=1)


    input_date = pd.to_datetime(res_date)
    lookup_days = int(arg1)
    inpdate=input_date.strftime("%Y-%m-%d")
    print(inpdate)
    #print((holiday_df['Date']==inpdate).any())
    while (((holiday_df['Date']==inpdate).any()==True) or (input_date.weekday()>4)):
        print("reached")
        #os.system('cp traded_pairs.csv marchIteration/traded_pairs_$d.csv')
        #os.system('mv traded_pairs.csv traded_pairs_last.csv')
        #os.system('mv traded_symbols.txt traded_symbols_last.txt')
        #os.system('rm final_symbols.txt')
        res_date += datetime.timedelta(days=1)
    ...
    os.system('mv traded_symbols.txt traded_symbols_last.txt')
    os.system('rm final_symbols.txt')
    #subprocess.Popen('cp traded_pairs.csv marchIteration/traded_pairs_$d.csv', shell=True)
    #subprocess.Popen('mv traded_pairs.csv traded_pairs_last.csv', shell=True)
    #subprocess.Popen('mv traded_symbols.txt traded_symbols_last.txt', shell=True)
    #subprocess.Popen('rm final_symbols.txt', shell=True)
